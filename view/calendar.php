<script>
    (function ($) {
        $(window).on('load', function () {
            <?php
            if (!function_exists('mg_teoola_load_calendar_events')) {
                function mg_teoola_load_calendar_events()
                {
                    $username = get_option("teoola_username") ? get_option("teoola_username") : '';
                    $password = get_option("teoola_key") ? get_option("teoola_key") : '';

                    $url = "https://api.teoola.com/json_entity_events.php?entity=$username&key=$password";
                    ...
                    if (count($response)) {
                        ...
                        datetime: new Date(" . $d["year"] . ", " . $goodM . ", " . $d["day"] . ", " . $d["hour"] . ", " . $d["minute"] . "),
            ...
            ?>
            $('#teoolaCalendar').teoolaCalendar({
                events: <?php mg_teoola_load_calendar_events(); ?>
            });
        });
